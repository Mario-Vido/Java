...
public class DivokyBill extends Karty {

    public DivokyBill() {
        this.name = "DivokyBill";
    }

    public void pouziKartuDivokyBill(int pozicia, HraciePoleZamierit hraciePoleZamierit, Hrac[] poleHracov, Inicializacia inicializacia, HraciePoleRybnik hraciePoleRybnik, int indexHracaNaTahu) {
        ...
        for (int i = 0; i < 3; i++) {
            if (poleHracov[indexHracaNaTahu].getKartyNaRuke().get(i).getName().equals(this.name)) {
                ...
                inicializacia.akcneKarty.add(new DivokyBill());
